...
def load_csv(filename: str) -> pd.DataFrame:
    file_path = SRC_DIR / filename if (SRC_DIR / filename).exists() else BASE_DIR / filename
...
# Data Ingestion
DATA = load_csv("reference.csv")
STAFF = load_csv("staff.csv")
KRI_DICTIONARY = load_csv("kri_dictionary.csv")
KRI_RESULT = load_csv("kri_result.csv") if (BASE_DIR / "kri_result.csv").exists() else pd.DataFrame()

KRI_DICTIONARY = KRI_DICTIONARY.rename(columns=str.strip).copy()
